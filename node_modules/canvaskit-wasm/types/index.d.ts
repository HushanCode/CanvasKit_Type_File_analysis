...
export interface CanvasKit {
    ...
    /**
     * 返回一个依靠c++存储的指定参数长度的类型数组，适用于那些能够很好的管理内存和初始化值的高级使用者。
     * 正确使用可以节省js和c++之间的数据复制
     * 错误使用会造成内存泄露
     * 所有用它应用的内存都需要CanvasKit.Free来释放
     * 
     * const mObj = CanvasKit.Malloc(Float32Array, 20);
     * 获得malloc内存周围的TypedArray视图(这不会复制任何东西)
     * const ta = mObj.toTypedArray();
     * 数据保存在变量中
     * const cf = CanvasKit.ColorFilter.MakeMatrix(ta); // mObj 也可以被使用.
     *
     * 最后
     * CanvasKit.Free(mObj);
     *
     * @param typedArray - typedArray的构造函数。
     * @param len - 要存储的*个元素的数量.
     */
    Malloc(typedArray: TypedArrayConstructor, len: number): MallocObj;

    /**
     类似Malloc但是设置GlyphIDs内存用的. 这个助手的存在是为了确保JS端和c++端，与GlyphIDs的宽度保持一致
     * @param len - 需要腾出空间的glyphid的数量。
     */
    MallocGlyphIDs(len: number): MallocObj;

    /**
     * Free释放Malloc返回的内存。
     * 由CanvasKit分配的任何内存。 需要CanvasKit.Free释放。
     */
    Free(m: MallocObj): void;

    // 曲面相关函数
    /**
     * 在给定的画布上创建一个Surface。 如果GPU和CPU模式都已编译, this
     将首先尝试创建一个GPU表面，然后在失败时回退到CPU表面。如果只是CPU模式已经编译，将创建一个CPU表面。
     * @param canvas - canvas元素本身或者带有它的DOM id的字符串。
    ...
    /**
     * 返回具有给定像素数据和格式的图像。
     * 注意，我们总是会复制像素数据，因为它的不一致GPU和CPU之间的行为(i.e. 像素数据将被转换成GPU纹理并且创建后不可修改)
     *
     * @param info
    ...
    /**
     * 返回一个由编码数据支持的图像, 但是尝试延迟解码直到图像实际使用/绘制。这种延迟允许系统缓存结果， 无论是在CPU或GPU上，这取决于图像绘制的位置。
     * 这种解码使用已编译到CanvasKit中的编解码器. If the bytes are
     * 如果这些字节数据是无效(或无法识别的编解码器)， 返回null. 查看更多细节 Image.h 
     * @param bytes
     */
    MakeImageFromEncoded(bytes: Uint8Array | ArrayBuffer): Image | null;

    /**
     * 返回一个带有提供的CanvasImageSource数据的图像 (e.g. <img>). 这将使用浏览器内置的编解码器 src路径的图片将被绘制到画布上，然后读取并放置到一个图像。
     * @param src
     */
    MakeImageFromCanvasImageSource(src: CanvasImageSource): Image;

    /**
     * 返回先前已序列化到给定字节的SkPicture。
     * @param bytes
     */
    ...
    /**
     * 返回一个基于给定位置和可选参数的顶点。
     * 更多细节SkVertices.h (especially the Builder) 
     * @param mode
     * @param positions
     * @param textureCoordinates
     * @param colors - 一个int颜色列表或者一个扁平的颜色数组
     * @param indices
     * @param isVolatile
     */
    ...
    /**
     * 返回由提供的json字符串构建的Skottie动画。
     * 要求Skottie被编译成CanvasKit。
     * @param json
     */
    MakeAnimation(json: string): SkottieAnimation;

    /**
     * 返回由提供的json字符串和资产构建的托管Skottie动画。
     * 要求Skottie被编译成CanvasKit。
     * @param json
     * @param assets - 一个命名blob的字典:{key: ArrayBuffer，…
     * @param filterPrefix - 可选字符串，用作选择“interesting”的名称筛选器
     *                       Lottie属性(出现在嵌入式播放器控件中)
     * @param soundMap - 声音标识符(字符串)到audioplayer的可选映射。
     *                   只有在动画支持声音时才需要。
     */
...
export interface Canvas extends EmbindObject<Canvas> {
    /**
     * 使用Src BlendMode填充当前剪辑的给定颜色。
     * 这样做的效果是用颜色替换剪辑中包含的所有像素。
     * @param color
     */
    ...
    /**
     *  在x轴上通过sx，在y轴上通过sy倾斜矩阵。 正值sx会随着y轴值的增加而向右倾斜 sy的正值倾斜，随着x轴值的增加而下降。
     * @param sx
     * @param sy
     */
